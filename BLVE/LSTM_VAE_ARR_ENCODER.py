...
from read_file_arr import Data_Hanlder
...
class LSTM_VAE(object):
    ...
    def _build_network(self):
        ...
        with tf.variable_scope('loss'):
            ...
            self.all_losses = 0.2*tf.reduce_sum(tf.square(self.X - self.recons_X),
                                            reduction_indices=reduce_dims) + \
                              0.8*tf.reduce_sum(tf.square(sigma_outputs - re_sigma_outputs) + \
    ...
    def train(self):
        ...
        for i in tqdm(range(self.train_iters),dynamic_ncols=True):
            ...
            if i % 2000 == 0:
                ...
                # saver.save(self.sess,'model/my-model',global_step=i)
    ...
    def judge(self, test,label):
        #load model
        # saver=tf.train.import_meta_graph('model/my-model-28000.meta')
        ...
        # rec_t = np.array(rec_loss).reshape((-1, 274))
        #
        # test_t = test.reshape((-1, 274))
        # # num=100
        # for num in [1, 4, 102, 110, 120, 121, 122, 131, 132, 133, 134, 135, 136, 137]:
...
def main():
    ...
    config['time_steps'] = 1
    config['z_dim'] = 1  # 潜在空间维度 default=64;如果要看latent space，z_dim=2or3
    config['n_hidden'] = 8 #todo：？default=8
    config['batch_size'] = 32  # 批梯度下降算法每次迭代都遍历批中的所有样本 default=32
    config['learning_rate'] = 1e-5  # 学习率 default=1e-5
    config['train_iters'] = 2000  # 1个iteration等于使用batchsize个样本训练一次 default=20000
    ...
    #保存config
    with open('config/arr.pickle','wb') as handle:
        pickle.dump(config,handle,protocol=pickle.HIGHEST_PROTOCOL)
        print('save seccess')

    #读取config
    with open('config/arr.pickle','rb') as handle:
