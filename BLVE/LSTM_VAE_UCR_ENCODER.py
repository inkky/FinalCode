...
class LSTM_VAE(object):
    ...
    # 统计测试机的异常得分并分标签1,-1
    def judge(self, test,label,dataname):
        ...
        # plt.savefig('img_lat/lat_'+dataname+'.png', dpi=300)
        #
        # plt.show()
    ...
    def plot_confusion_matrix(self):
        ...
        predict_label = self.judge(self.data_source.test,self.data_source.test_label,self.dataname)
...
def write_result(dataname, test_label, predict_label, loss, ano_ratio, config):
    with open('result/latent_ucr.txt', 'a') as result:
...
def main():
    ...
    config['time_steps'] = 1
    config['z_dim'] = 1  # 潜在空间维度 default=16;latent=3
    config['n_hidden'] = 32  #default=16
    config['batch_size'] = 40  # 批梯度下降算法每次迭代都遍历批中的所有样本数量 （在合理范围内增大Batch_Size）default=40
    config['learning_rate'] = .005  # 学习率default=0.005
    config['train_iters'] = 2000  # 1个iteration等于使用batchsize个样本训练一次default=4000
    config['if_train'] = True
    config['epoch'] = 200  # 1个epoch等于使用训练集中的全部样本训练一次 default=100
    config['outlier_fraction'] = 0.5
    config['dataname']=['Herring.tsv']
    ...
    # 》》》》》》单个数据集
    # dataname = ['Herring.tsv']
    for i, v in enumerate(config['dataname']):
        tf.reset_default_graph() #清空default graph 和 node
