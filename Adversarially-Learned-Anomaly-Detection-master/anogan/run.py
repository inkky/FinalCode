...
def display_parameters(batch_size, starting_lr, ema_decay,
    ...
    print('data set shape:',datashape)
import datetime
...
def train_and_test(dataset, nb_epochs, weight, degree, random_seed, label,
    ...
    # Placeholders
    x_pl = tf.placeholder(tf.float32, shape=(None,trainx.shape[1]), name="input")
    ...
    display_parameters(batch_size, starting_lr, ema_decay, weight, degree, label,trainx.shape[1])
    ...
    # EMA
    x_gen_ema = gen(x_pl,random_z, is_training=is_training_pl,
                    getter=get_getter(gen_ema), reuse=True)
    rec_x_ema = gen(x_pl,z_optim, is_training=is_training_pl,
                        getter=get_getter(gen_ema), reuse=True)
