...
def display_parameters(batch_size, starting_lr, ema_decay, degree, label,
                       allow_zz, score_method, do_spectral_norm,datashape):
    """See parameters
    """
...
def train_and_test(dataset, nb_epochs, degree, random_seed, label,
    ...
    network = importlib.import_module('alad.{}_utilities'.format(dataset)) #调用了_utilities
    ...
    ema_decay = 0.999 #TODO：？

    global_step = tf.Variable(0, name='global_step', trainable=False)

    trainx, trainy = data.get_train(label)  # 这个label并没有被用到？？？y！

    # Placeholders
    x_pl = tf.placeholder(tf.float32, shape=(None,trainx.shape[1]), #(None,数据的大小)
                          name="input_x")
    z_pl = tf.placeholder(tf.float32, shape=[None, latent_dim], #32
                          name="input_z")
    ...
    display_parameters(batch_size, starting_lr, ema_decay, degree, label,
                       allow_zz, score_method, do_spectral_norm,trainx.shape[1])
    ...
    with tf.variable_scope('generator_model'):
        x_gen = gen(x_pl,z_pl, is_training=is_training_pl)
        rec_x = gen(x_pl,z_gen, is_training=is_training_pl, reuse=True)
    ...
    with tf.name_scope('loss_functions'):
        # one_like 返回一个具有和给定tensor相同形状（shape）和相同数据类型（dtype），但是所有的元素都被设置为1的tensor。
    ...
    with tf.variable_scope('generator_model'):
        rec_x_ema = gen(x_pl,z_gen_ema, is_training=is_training_pl,
                              getter=get_getter(gen_ema), reuse=True)
        x_gen_ema = gen(x_pl,z_pl, is_training=is_training_pl,
                              getter=get_getter(gen_ema), reuse=True)
