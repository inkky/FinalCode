...
from AL_BLVE.read_arr import Data_Hanlder
...
def main():
    ...
    config['time_steps'] = 1
    config['z_dim'] = 64  # 潜在空间维度 default=64;如果要看latent space，z_dim=2or3
    config['n_hidden'] = 16 #todo：？default=8
    config['batch_size'] =6  # 批梯度下降算法每次迭代都遍历批中的所有样本 default=32
    config['learning_rate'] = 3e-4  # 学习率 default=1e-5
    config['train_iters'] = 120  # 1个iteration等于使用batchsize个样本训练一次 default=200
    ...
    # #读取config
    # with open('para_util/arrhythmia_para.pickle','rb') as handle:
