...
from activelearning.modaltest.read_arr_random import Data_Hanlder
...
class LSTM_VAE(object):
    ...
    def _build_network(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            with tf.variable_scope('lat_sigma'):
                ...
                # sampleing gussian
                self.sample_Z = self.mu_outputs + sigma_outputs * tf.random_normal(
    ...
    '''主动学习'''
    # todo：修改train和test，每个train_iter，都进行一次train和test，
    #  得到的结果和原始数据进行对比trend，选出lof最小的点，
    #  再加入到训练集中
    #  重复
    ...
    def _arange_score(self, input_data): #todo:异常得分归一化
        ...
        self.anomaly_score=np.percentile(input_all_losses,(1-self.true_ano)*100+2)
    ...
    def predict(self,test):
        loss = self.sess.run([self.all_losses], feed_dict={self.X: test})
        loss=list(np.array(loss).reshape(-1))
        # print(loss)
        query_idx=loss.index(max(loss))
        print(query_idx)
        return query_idx

    def query_idx(self):
        # labeled_idx=self.data_source.init_query_idx()
        labeled_idx=[self.predict(self.data_source.unlabeled_data)]
        print(labeled_idx)
        print(self.data_source.train.shape)
        print(self.data_source.unlabeled_data[labeled_idx].shape)
        ...
        print(normal)
        # print(self.data_source.train.shape)
        # print(self.data_source.unlabeled_data[normal].shape)
        self.data_source.train = np.vstack((self.data_source.train, self.data_source.unlabeled_data[normal]))
        self.data_source.train_label = np.hstack(
            (self.data_source.train_label, self.data_source.unlabeled_label[normal]))
    ...
    def judge(self, test,label):
        ...
        # plt.savefig('img_lat/lat_arr2.png', dpi=300)
        #
        # plt.show()
        ...
        # point=np.reshape(point,(113,274))
        # np.set_printoptions(threshold=np.inf)
        # with open('result/pointanomaly_arr.txt', 'a') as result:
        #     result.write(str(point))



        # z=self.sess.run([self.sample_Z],feed_dict={self.X:test})
...
def main():
    ...
    config['time_steps'] = 1
    config['z_dim'] = 3  # 潜在空间维度 default=64;如果要看latent space，z_dim=2or3
    config['n_hidden'] = 8 #todo：？default=8
    config['batch_size'] =1  # 批梯度下降算法每次迭代都遍历批中的所有样本 default=32
    config['learning_rate'] = 3e-4  # 学习率 default=1e-5
    config['train_iters'] = 100  # 1个iteration等于使用batchsize个样本训练一次 default=20000
    config['num_layers'] = 3  # number of layers of stacked RNN's default=3
    config['if_train'] = True
    config['epoch'] = 200  # 1个epoch等于使用训练集中的全部样本训练一次 default=1000
    ...
    with open('result/lstm_vae_encoder_arr28.txt', 'a') as result:
        lstm_vae=LSTM_VAE('arrhythmia',config)
        # lstm_vae.train()
        lstm_vae.active_train()
        test_label, predict_label,loss,ano_ratio = lstm_vae.plot_confusion_matrix()
        # result.write(str(test_label))
        # result.write('\n')
        # result.write(str(predict_label))
        # result.write('\n')
