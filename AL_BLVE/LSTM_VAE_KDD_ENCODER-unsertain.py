...
from activelearning.modaltest.read_kdd_random import Data_Hanlder
...
class LSTM_VAE(object):
    ...
    def query_idx(self):
        ...
        # train = np.vstack((self.data_source.train, self.data_source.all_data[labeled_idx]))
        # self.data_source.train_label = np.vstack((self.data_source.train_label, self.data_source.all_labels[labeled_idx]))
    ...
    def judge(self, test,label):
        ...
        # # plt.figure()
        # # for i in range(30):
        # #     if self.data_source.unlabeled_label[i]==-1:#anomaly
        # #         plt.plot(test_t[i])
        # # plt.show()
...
def main():
    ...
    config['z_dim'] = 3  # 潜在空间维度 default=32； 如果要看latent space，z_dim=2or3
    config['n_hidden'] = 8 #todo：？ default=8
    config['batch_size'] = 1  # 批梯度下降算法每次迭代都遍历批中的所有样本 default=15
    config['learning_rate'] = 2e-4  # 学习率 default=2e-4
    config['train_iters'] = 100  # 1个iteration等于使用batchsize个样本训练一次 default=100
