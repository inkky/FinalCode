...
import importlib
import sys
import bigan.ucr_utilities as network
import data.ucr as data
...
def create_logdir(method, weight, rd):
    ...
    return "bigan/train_logs/arr/{}/{}/{}/{}/".format(weight, method, rd,time)
...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
    ...
    """
    logger = logging.getLogger("BiGAN.train.ucr.{}".format(method))
    ...
    with tf.variable_scope('generator_model'):
        ...
        x_gen = gen(input_pl,z, is_training=is_training_pl)
    ...
    with tf.variable_scope('generator_model'):
        reconstruct_ema = gen(input_pl,z_gen_ema, is_training=is_training_pl,
