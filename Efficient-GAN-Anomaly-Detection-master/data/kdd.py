...
def _get_dataset():
    ...
    labels[labels != 'normal.'] = 0 #异常
    labels[labels == 'normal.'] = 1 #正常

    df['label'] = labels
    print(np.where(df['label']==0))
...
def _encode_text_dummy(df, name):
    ...
    进行one-hot编码
    """
    dummies = pd.get_dummies(df.loc[:,name])# df.loc选取特定列，将name进行one-hot编码

    for x in dummies.columns: #['icmp', 'tcp', 'udp']
        dummy_name = "{}-{}".format(name, x)
        df.loc[:, dummy_name] = dummies[x]
    df.drop(name, axis=1, inplace=True)#删除表的某一列1，默认行
