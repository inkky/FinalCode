...
def load_data(opt):
    ...
    if opt.dataset in ['cifar10']:
        ...
        print('dataset train type = ' + str(type(dataset['train'])))

        dataset['train'].data, dataset['train'].targets, dataset['test'].data, dataset['test'].targets = get_mnist_anomaly_dataset(
        ...
        # print('data shape testing=========={}'.format(Variable(dataset['train'].data).data.size()))
        dataloader = {x: torch.utils.data.DataLoader(dataset=dataset[x],
                                                     batch_size=opt.batchsize,
                                                     shuffle=shuffle[x],
                                                     num_workers=int(opt.workers),
                                                     drop_last=drop_last_batch[x]) for x in splits}
        # a = dataloader['train']
        # print('data shape testing=========={}'.format(Variable(a).data.size()))
        return dataloader
