...
class Ganomaly(object):
    ...
    def __init__(self, opt, dataloader=None):
        ...
        self.input = torch.empty(size=(self.opt.batchsize, 3, self.opt.isize, self.opt.isize), dtype=torch.float32, device=self.device)     # isize = imput image size
        self.label = torch.empty(size=(self.opt.batchsize,), dtype=torch.float32, device=self.device)
        self.gt    = torch.empty(size=(opt.batchsize,), dtype=torch.long, device=self.device)       # GroundTruth
    ...
    def set_input(self, input):
        ...
        """
        self.input.resize_(input[0].size()).copy_(input[0])     # 在每个epoch的每个batch，都根据这个batch的维度重新设置输入Tensor的维度
        self.gt.resize_(input[1].size()).copy_(input[1])           # 所以前面的类的构造方法中的初始化在训练阶段没什么实际作用（只起到一个占位的作用）
    ...
    def update_netd(self):
        ...
        # print(self.out_d_real.size())
        # print(self.feat_real.size())
        # --
        # Train with fake
        self.label.data.resize_(self.opt.batchsize).fill_(self.fake_label)
        self.fake, self.latent_i, self.latent_o = self.netg(self.input)
        # print(self.fake.size())
        self.out_d_fake, self.feat_fake = self.netd(self.fake.detach())
        # --
        print(self.input.shape)
        print(self.fake.detach().shape)
        print(self.feat_real.shape)
        print(self.feat_fake.shape)
        exit()
    ...
    def update_netg(self):
        ...
        # print(self.out_g.size())
        self.err_g_bce = self.bce_criterion(self.out_g, self.label)
        # print('self.fake.size() = {}'.format(self.fake.size()))
        # print('self.input.size() = {}'.format(self.input.size()))
    ...
    def train_epoch(self):
        ...
        for data in tqdm(self.dataloader['train'], leave=False, total=len(self.dataloader['train'])):
            ...
            inputs, labels = data
            # print('======Mnist data shape   {}      {}'.format(Variable(inputs).data.shape, Variable(labels).data.shape))
            self.set_input(data)
            self.optimize()
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):     # start epoch      total number of epoch
            # Train for one epoch
            self.train_epoch()
            res = self.test()       # 每个epoch完了之后test，相当于验证
            if res['AUC'] > best_auc:
                best_auc = res['AUC']
                self.save_weights(self.epoch)
            self.visualizer.print_current_performance(res, best_auc)
        print(">> Training model %s.[Done]" % self.name())
        print("TSNE training samples corrsponding Z.......")
